    ...
    public boolean onError(MediaPlayer mp, int whatError, int extra) {
        try {
            switch (whatError) {
                ...
                break;
                case MediaPlayer.MEDIA_ERROR_MALFORMED: {//比特流是不符合先关编码标准的文件
                    Toast.makeText(getContext(), "文件编码不正确!", Toast.LENGTH_SHORT).show();
                }
                break;
                case MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK: {//视频流是其容器不是有效的渐进式播放即视频的索引（例如，MOOV原子）不是在文件的开头。
                    Toast.makeText(getContext(), "非有效的视频文件!", Toast.LENGTH_SHORT).show();
                }
                break;
                case MediaPlayer.MEDIA_ERROR_SERVER_DIED: {//媒体播放器死亡,通常这种情况需要释放MediaPlay和重新实例化一个对象
                    restar();
                }
                break;
                case MediaPlayer.MEDIA_ERROR_TIMED_OUT: {//有些操作需要太长时间才能完成,一般超过3-5秒
                    restar();
                }
                break;
                case MediaPlayer.MEDIA_ERROR_UNKNOWN: {//未指定媒体播放器的错误,不进行重连
                    //重新释放
                ...
                break;
                case MediaPlayer.MEDIA_ERROR_UNSUPPORTED: {//比特流是符合相关的编码标准或文件规范,但媒体框架不支持该功能
